...
##### 操作步驟：
1. 啟動程式後，瀏覽器會在背景最小化執行，請直接在終端輸入即可，不會打斷操作
2. 根據程式的要求，在終端輸入您的起始站、終點站、日期和起迄時間
3. 若查無符合條件的班次，終端會顯示「查無對應時刻表」的提示
4. 選擇能夠訂票的車次
5. 根據程式的要求，在終端輸入您的身分證字號
6. 此時瀏覽器視窗會自動跳到最前面，請輸入驗證碼後，回到終端按下 Enter 鍵
7. 程式會自動判斷結果，終端顯示「訂票成功」或「訂票失敗」